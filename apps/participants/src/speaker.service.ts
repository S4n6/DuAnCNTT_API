...
export class SpeakerService {
    ...
    async findOne(id: ObjectId): Promise<SpeakerResponse> {
        try {
            ...
            if (!speaker) {
                return new SpeakerResponse(false, `Speaker with ID ${id} not found`, []);
            }
    ...
    async create(speakerDto: SpeakerDto): Promise<SpeakerResponse> {
        try {
            ...
            return new SpeakerResponse(true, 'Speaker created successfully', speaker);
        } catch (error) {
    ...
    async update(id: ObjectId, speakerDto: SpeakerDto): Promise<SpeakerResponse> {
        try {
            const updatedSpeaker: SpeakerDto = await this.speakerModel.findByIdAndUpdate(
                id,
                speakerDto,
                {
                    new: true,
            ...
            if (!updatedSpeaker) {
                return new SpeakerResponse(false, `Speaker with ID ${id} not found`, null);
            }
